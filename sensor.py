"""Sensor to collect the reference daily prices of electricity ('EE')."""
...
from .data_provider import EEData
...
async def async_setup_entry(
    ...
    ee_data_handler = EEData(
        local_timezone=hass.config.time_zone,
        websession=async_get_clientsession(hass),
        timeout=_DEFAULT_TIMEOUT,
    )
    async_add_entities(
        [EEPriceSensor(name, config_entry.unique_id, ee_data_handler)], False
    )


class EEPriceSensor(RestoreEntity, SensorEntity):
    ...
    def __init__(self, name, unique_id, ee_data_handler):
        """Initialize the sensor object."""
        self._name = name
        self._unique_id = unique_id
        self._ee_data = ee_data_handler
        self._num_retries = 0
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if state := await self.async_get_last_state():
            self._ee_data.state = state.state
        ...
        _LOGGER.debug(
            "Setup of price sensor %s (%s), " "updating prices each hour at %s min",
            self.name,
            self.entity_id,
            mins_update,
        )
    ...
    def native_value(self) -> float:
        ...
        return self._ee_data.state

    @property
    def available(self) -> bool:
        """Return True if entity is available."""
        return self._ee_data.state_available

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return the state attributes."""
        return self._ee_data.attributes

    @callback
    def update_current_price(self, now):
        """Update the sensor state, by selecting the current price for this hour."""
        self._ee_data.process_state_and_attributes(now)
        self.async_write_ha_state()

    async def async_update_prices(self, now):
        """Update electricity prices from the ESIOS API."""
        prices = await self._ee_data.async_update_prices(now)
        if not prices and self._ee_data.source_available:
            self._num_retries += 1
            if self._num_retries > 2:
                ...
                self._ee_data.source_available = False
                return

            retry_delay = 2 * self._num_retries * self._ee_data.timeout
            _LOGGER.debug(
        ...
        self._num_retries = 0
        if not self._ee_data.source_available:
            self._ee_data.source_available = True
